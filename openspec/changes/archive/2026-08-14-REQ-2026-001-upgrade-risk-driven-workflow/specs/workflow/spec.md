...
The CLI SHALL dry-run readiness, merge delta clauses by stable ID, apply explicit retention, and archive a change without leaving partially applied current specs.

#### Scenario `SC-workflow-005`: Close precondition fails

- **WHEN** a task, acceptance item, evidence, or merge baseline is not ready
- **THEN** close SHALL fail before mutating current specs or moving the change
